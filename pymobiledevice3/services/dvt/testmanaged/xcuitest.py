...
class ReverseRemoteService:
    ...
    async def __on_method_call__(self, selector: str, args: Optional[MessageAux] = None):
        ...
        if not hasattr(self, _selector):
            ...
            await self.service.send_reply_error(self.channel, self.service.cur_remote_message)
            return
        ret = getattr(self, _selector)(args)
        await self.service.send_reply(self.channel, self.service.cur_remote_message, ret)
    ...
    async def consume(self):
        ...
        try:
            ...
            while self.__running__:
                key, value = await self.main_dvt.recv_plist(self.main_chan)
                if key == "_XCT_didFinishExecutingTestPlan":
                    self.__closing__ = True
                await self.listener.__on_method_call__(key, value)
